...
public class AuthToolService {
  ...
  @McpTool(name = "authenticate_user", description = "Authenticate a user with username and password and fetch back the user details.")
  public Token authenticateUser(@McpToolParam(description = "Username of the user") String username,
      @McpToolParam(description = "Password of the user") String password) {
    return this.authenticationApi.loginApiAuthLoginPost(new UserLogin().username(username).password(password));
  }

  @McpTool(name = "get_current_user_info", description = "Get the current logged in user details using the token")
